...
st.sidebar.caption("Ensemble v2.1: Normalized Engine + Context-Aware Windowing")
...
# Clean Text Normalizer (No regex escaping bugs!)
def normalize_text(text):
    if not text:
        return ""
    # split() automatically handles \n, \t, \r, and collapses multi-spaces cleanly
...
def predict_ai_prob(text_chunk):
    ...
    clean_chunk = normalize_text(text_chunk)
    if len(clean_chunk.split()) < 3:
        return 20.0
    try:
        res = detector_pipeline(clean_chunk[:512])[0]
...
st.write("Advanced hybrid analysis with normalized PDF parsing and calibrated context-aware scoring.")

input_method = st.radio("Input method:", ["Paste Text", "Upload Document"])
raw_input_text = ""

if input_method == "Paste Text":
    raw_input_text = st.text_area("Paste text here:", placeholder="Paste your text (minimum 20-30 words recommended)...", height=200)
else:
    uploaded_file = st.file_uploader("Upload document", type=["pdf", "docx", "txt"])
    if uploaded_file is not None:
        file_ext = uploaded_file.name.split(".")[-1].lower()
        if file_ext == "pdf":
            raw_input_text = extract_text_from_pdf(uploaded_file)
        elif file_ext == "docx":
            raw_input_text = extract_text_from_docx(uploaded_file)
        elif file_ext == "txt":
            raw_input_text = normalize_text(uploaded_file.getvalue().decode("utf-8"))

if st.button("Analyze Text", type="primary"):
    clean_text = normalize_text(raw_input_text)
    ...
    if word_count < 15:
        ...
        with st.spinner("Running Calibrated Scan..."):
            sentence_details = []
            ai_sentence_count = 0
            
            # 1. Context-Aware Sentence Scan
