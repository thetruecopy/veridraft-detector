...
# --- Production Scoring Selector ---
# Rollback path: change ONLY this line from "v2.1" to "legacy" and redeploy.
PRODUCTION_SCORER = "v2.1"
PRODUCT_BASELINE = "PB-1"
VALID_PRODUCTION_SCORERS = {"legacy", "v2.1"}

if PRODUCTION_SCORER not in VALID_PRODUCTION_SCORERS:
    raise ValueError(
        f"Invalid PRODUCTION_SCORER: {PRODUCTION_SCORER}. "
        f"Choose one of: {sorted(VALID_PRODUCTION_SCORERS)}"
    )
...
def programmatic_analyze_text(
    ...
    Public score/classification follows PRODUCTION_SCORER.
    Legacy and experimental values remain available under diagnostics.
    """
    ...
    (
        ...
        sentence_data,
        diagnostics,
    ) = analyze_document(cleaned_text, model_pairs)

    production_score, production_band, classification = resolve_production_score(
        legacy_final_ai,
        diagnostics,
        burstiness,
        ttr,
    ...
    api_diagnostics = dict(diagnostics)
    api_diagnostics.update(
        {
            "production_scorer": PRODUCTION_SCORER,
            "production_score": production_score,
            "production_band": production_band,
            "legacy_final_ai": float(legacy_final_ai),
            "candidate_v21_probability": float(candidate_v21_probability),
            "candidate_v21_band": candidate_v21_band,
        }
    )

    return {
        "overall_ai_score_percent": round(production_score * 100, 2),
        ...
        "sentences": [
            {
                "sentence": s,
                "legacy_sentence_ai_score_percent": round(score * 100, 2),
            }
            for s, score in sentence_data
        ],
        "sentence_map_note": (
            "Sentence-level map remains on the preserved legacy sentence scorer "
            "during the v2.1 production transition. Document-level public score "
            f"uses {PRODUCTION_SCORER}."
        ),
    }
...
def legacy_band(score, high_threshold, low_threshold):
    """Legacy band using the existing sidebar thresholds."""
    if score >= high_threshold:
        return "AI"
    if score >= low_threshold:
        return "Mixed"
...
def production_classification_text(band):
    """User-facing classification text for the active production scorer."""
    if band == "AI":
...
def resolve_production_score(
    ...
    """
    Route the public document-level result through the selected production scorer.

    Rollback is one line:
        PRODUCTION_SCORER = "legacy"

    Candidate v2.1 keeps its frozen 50% / 65% bands and therefore does not
    use the legacy threshold sliders.
    """
    legacy_score = float(legacy_final_ai)

    if PRODUCTION_SCORER == "v2.1":
        score, band = candidate_v21_score(diagnostics, burstiness, ttr)
    else:
        score = legacy_score
        band = legacy_band(score, high_threshold, low_threshold)

    return float(score), str(band), production_classification_text(band)
...
def render_calibration_lab(active_models, high_threshold, low_threshold):
    ...
    if st.button("Run Calibration", type="primary", key="run_calibration"):
        ...
        for idx, row in calibration_df.reset_index(drop=True).iterrows():
            ...
            try:
                ...
                production_score, production_band, _ = resolve_production_score(
                    current_final,
                    diagnostics,
                    burstiness,
                    ttr,
                ...
                results.append(
                    {
                        ...
                        "production_correct_strict": (
                            (actual_label == "AI" and production_band == "AI")
                            or (actual_label == "Human" and production_band == "Human")
                        ),
                        "text": sample_text,
                    }
                )
    ...
    production_metrics = build_band_metrics(
        valid_results, "production_band"
    )
    ...
    metric_table = pd.DataFrame(
        [
            ...
            {
                "Scoring": f"Production ({PRODUCTION_SCORER})",
                "Strict Accuracy": production_metrics["strict_accuracy"],
                "AI Recall": production_metrics["ai_recall"],
                "Human Specificity": production_metrics["human_specificity"],
                "False Positives": production_metrics["false_positives"],
                "False Negatives": production_metrics["false_negatives"],
                "AI → Mixed": production_metrics["mixed_ai"],
                "Human → Mixed": production_metrics["mixed_human"],
            },
        ]
    )
    ...
    st.markdown(f"#### Production ({PRODUCTION_SCORER}) Confusion Matrix")
    st.dataframe(production_metrics["confusion"], use_container_width=True)
...
# --- Public / Internal Tool Gate ---
# Public behavior is the default. Internal controls are exposed only when the
# Streamlit secret INTERNAL_TOOLS_ENABLED is explicitly set to true.
#
# Streamlit secrets example:
# INTERNAL_TOOLS_ENABLED = true
#
# Remove the secret or set it to false before external PB-1 testing.
try:
    _internal_secret = st.secrets.get("INTERNAL_TOOLS_ENABLED", False)
except Exception:
    _internal_secret = False

if isinstance(_internal_secret, bool):
    INTERNAL_TOOLS_ENABLED = _internal_secret
else:
    INTERNAL_TOOLS_ENABLED = str(_internal_secret).strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
# Keep stable legacy defaults even when the legacy controls are hidden.
high_threshold = 0.65
low_threshold = 0.35
show_api_tab = False

st.sidebar.title("⚙️ VeriDraft")

if INTERNAL_TOOLS_ENABLED:
    st.sidebar.success("🔒 Internal tools enabled")
    st.sidebar.caption(f"Product baseline: {PRODUCT_BASELINE}")

    st.sidebar.markdown("---")
    st.sidebar.subheader("Internal Detection Settings")

    if PRODUCTION_SCORER == "v2.1":
        st.sidebar.success("Production scorer: Candidate v2.1")
        st.sidebar.caption(
            "Frozen v2.1 bands: Human <50% · Mixed 50–64.99% · AI ≥65%. "
            "Legacy thresholds below affect legacy diagnostics only."
        )
    else:
        st.sidebar.info("Production scorer: Legacy")

    high_threshold = st.sidebar.slider(
        "Legacy High AI Likelihood Cutoff (%)",
        50,
        90,
        65,
        key="internal_high_threshold",
    ) / 100.0
    low_threshold = st.sidebar.slider(
        "Legacy Human Likelihood Cutoff (%)",
        10,
        49,
        35,
        key="internal_low_threshold",
    ) / 100.0

    st.sidebar.markdown("---")
    st.sidebar.subheader("🔒 Developer Access")
    show_api_tab = st.sidebar.checkbox(
        "Enable Developer / API View",
        value=False,
        key="internal_developer_view",
    )

    st.sidebar.markdown("---")
    st.sidebar.subheader("📌 Model Specs")
    st.sidebar.caption(
        "Ensemble: RoBERTa OpenAI + ChatGPT Detector + Fakespot diagnostic"
    )
    st.sidebar.caption(
        "Engine: Chunked Processing + Evasion Shield + Enterprise API"
    )

    st.sidebar.markdown("---")
    st.sidebar.caption(
        "VeriDraft · PB-1 Product Baseline · Internal Mode"
    )
else:
    st.sidebar.caption("PB-1 Product Baseline")
    st.sidebar.caption(
...
if st.button("Analyze Text", type="primary"):
    ...
    if len(words) < MIN_WORD_COUNT:
        ...
        production_score, production_band, production_classification = (
            resolve_production_score(
                legacy_final_ai,
                diagnostics,
                burstiness,
                ttr,
        ...
        # "ai_prob" remains the public active score for compatibility.
        st.session_state["ai_prob"] = production_score
        st.session_state["production_score"] = production_score
        st.session_state["production_band"] = production_band
        st.session_state["production_classification"] = production_classification
        st.session_state["production_scorer"] = PRODUCTION_SCORER
        st.session_state["legacy_final_ai"] = legacy_final_ai
...
if "ai_prob" in st.session_state:
    ...
    production_score = st.session_state.get("production_score", ai_prob)
    production_band = st.session_state.get(
        "production_band",
        legacy_band(ai_prob, high_threshold, low_threshold),
    )
    production_classification = st.session_state.get(
        "production_classification",
        production_classification_text(production_band),
    )
    production_scorer = st.session_state.get("production_scorer", "legacy")
    legacy_final_ai = st.session_state.get("legacy_final_ai", ai_prob)
    ...
    with tab1:
        ...
        col1.metric("Overall AI Score", f"{production_score * 100:.1f}%")
        ...
        if show_api_tab:
            ...
            d1, d2, d3, d4, d5, d6, d7, d8, d9, d10, d11 = st.columns(11)
            ...
            d8.metric("Legacy Final", f"{legacy_final_ai * 100:.1f}%")
            ...
            d11.metric(
                "Production",
                f"{production_score * 100:.1f}% · {production_band}",
            )
            st.caption(
                f"Active production scorer: `{production_scorer}` · "
                "Rollback: change `PRODUCTION_SCORER = \"legacy\"` and redeploy."
            )

        st.markdown("#### Score Interpretation")
        if production_band == "AI":
            st.error(
                "⚠️ **High Probability of AI Generation:** The active production scorer "
                "found strong AI-generation signals."
            )
        elif production_band == "Mixed":
            st.warning(
                "⚡ **Mixed Signals Detected:** The active production scorer found "
                "insufficient evidence for a confident AI or Human classification."
            )
        else:
            st.success(
                "✅ **Likely Human-Written:** The active production scorer found "
                "insufficient evidence to classify the text as AI-generated."
            )

        # --- PB-1 Visible Feedback CTA ---
        # This sits directly below the result so users do not have to discover
        # the Export & Feedback tab before telling us when a result looks wrong.
        st.markdown("### 💬 Was this result accurate?")
        st.info(
            "If this result looks right — or especially if it looks wrong — please tell us. "
            "It takes about 15 seconds. VeriDraft does **not** store the text you analyzed."
        )

        with st.form("pb1_inline_feedback_form", clear_on_submit=True):
            inline_accuracy = st.radio(
                "Did VeriDraft get this result right?",
                ["Yes", "No", "Not sure"],
                horizontal=True,
                key="pb1_inline_accuracy",
            )

            inline_ground_truth = st.selectbox(
                "Do you know how this text was created?",
                ["Unknown", "Human", "AI"],
                key="pb1_inline_ground_truth",
            )

            inline_notes = st.text_area(
                "Optional: tell us what looked right or wrong",
                placeholder=(
                    "A short note is enough. Please do not paste the analyzed text "
                    "or include personal information."
                ),
                key="pb1_inline_notes",
                max_chars=500,
            )

            inline_privacy_confirmed = st.checkbox(
                "I understand that VeriDraft will save my feedback and non-text diagnostic "
                "metadata for product evaluation, but not the text I analyzed.",
                key="pb1_inline_privacy_confirmed",
            )

            inline_submit = st.form_submit_button(
                "Submit quick feedback",
                type="primary",
            )

        if inline_submit:
            if not inline_privacy_confirmed:
                st.warning("Please confirm the privacy notice before submitting.")
            else:
                inline_case_id = f"PB1-{uuid.uuid4().hex[:12].upper()}"
                inline_evidence_record = {
                    "schema_version": "pb1_inline_v1",
                    "feedback_source": "inline_result_cta",
                    "case_id": inline_case_id,
                    "submitted_at_utc": datetime.now(timezone.utc).isoformat(),
                    "result_accurate": inline_accuracy,
                    "ground_truth": inline_ground_truth,
                    "feedback_notes": inline_notes.strip(),
                    "word_count": len(target_text.split()),
                    "production_scorer": production_scorer,
                    "production_score": round(float(production_score), 6),
                    "production_band": production_band,
                    "legacy_score": round(float(legacy_final_ai), 6),
                    "burstiness": round(float(burstiness), 6),
                    "perplexity": round(float(perplexity), 6),
                    "ttr_percent": round(float(ttr), 6),
                    "model1_ai": round(float(diagnostics.get("model1_avg", 0.0)), 6),
                    "model2_ai": round(float(diagnostics.get("model2_avg", 0.0)), 6),
                    "fakespot_ai": round(float(diagnostics.get("model3_avg", 0.0)), 6),
                }
                ...
                try:
                    (
                        ...
                        .insert(
                            {
                                "actual_label": inline_ground_truth,
                                "predicted_score": float(production_score),
                                "burstiness_cv": float(burstiness),
                                "user_notes": "[PB1-INLINE] " + json.dumps(
                                    inline_evidence_record,
                                    ensure_ascii=False,
                                    separators=(",", ":"),
                                ),
                                # Privacy rule: analyzed text/snippets are never persisted.
                                "text_snippet": None,
                            }
                        )
                        .execute()
                    ...
                    st.success(
                        f"Thank you — quick feedback saved as {inline_case_id}. "
                        "The analyzed text was not stored."
                    )
                except Exception as e:
                    st.error(f"Failed to save quick feedback: {e}")

    with tab2:
        st.subheader("Sentence-Level AI Map")
        st.caption(
            "Sentence-level highlighting currently uses the preserved legacy sentence scorer. "
            "The document-level Overall AI Score and classification use the active production "
            f"scorer: {PRODUCTION_SCORER}."
        )
        ...
        st.markdown(
            f"<br>Legacy sentence map: 🔴 **High AI** (≥{int(high_threshold * 100)}%) | "
    ...
    with tab3:
        ...
        st.caption(
            f"Document production result: {production_score * 100:.1f}% · "
            f"{production_band} · scorer={production_scorer}"
        )

        csv_buffer = io.StringIO()
        writer = csv.writer(csv_buffer)
        writer.writerow(["Report_Type", "Value_1", "Value_2"])
        writer.writerow(
            [
                "Document Production Result",
                f"{production_score * 100:.2f}",
                f"{production_band} ({production_scorer})",
            ]
        )
        writer.writerow(
            [
                "Legacy Document Result",
                f"{legacy_final_ai * 100:.2f}",
                "legacy",
            ]
        )
        writer.writerow([])
        writer.writerow(["Sentence", "Legacy_AI_Probability_Percent", ""])
        for sent, score in sentence_data:
        ...
        st.markdown("---")
        st.subheader("PB-1 Product Evidence")
        st.caption(
            "Help us understand whether VeriDraft is useful in real situations. "
            "Your submitted evidence does not store the text you analyzed."
        )
        ...
        ground_truth = st.selectbox(
            "Do you know how this text was created?",
            ["Unknown", "Human", "AI"],
            key="pb1_ground_truth",
        )

        ai_generator = "Not applicable"
        ai_editing = "Not applicable"
        if ground_truth == "AI":
            ai_generator = st.selectbox(
                "Which AI tool created it?",
                ["ChatGPT", "Gemini", "Grok", "Claude", "Other", "Unknown"],
                key="pb1_ai_generator",
            )
            ai_editing = st.selectbox(
                "Was the AI text edited afterward?",
                ["No", "Light editing", "Heavy editing", "Unknown"],
                key="pb1_ai_editing",
            )
        ...
        confusing = st.radio(
            "Was anything about the result confusing?",
            ["No", "Yes"],
            horizontal=True,
            key="pb1_confusing",
        )
        ...
        feedback_notes = st.text_area(
            "Optional feedback",
            placeholder=(
                "Tell us what was useful or confusing. "
                "Please do not paste the text you analyzed or include personal information."
            ),
            key="pb1_feedback_notes",
            max_chars=1000,
        )
        ...
        privacy_confirmed = st.checkbox(
            "I understand that VeriDraft will save my feedback and non-text diagnostic "
            "metadata for product evaluation, but not the text I analyzed.",
            key="pb1_privacy_confirmed",
        )

        if st.button("Submit Product Feedback", key="submit_pb1_feedback"):
            if not privacy_confirmed:
                st.warning("Please confirm the privacy notice before submitting.")
            else:
                case_id = f"PB1-{uuid.uuid4().hex[:12].upper()}"
                evidence_record = {
                    "schema_version": "pb1_v1",
                    "case_id": case_id,
                    "submitted_at_utc": datetime.now(timezone.utc).isoformat(),
                    "helpful": helpful,
                    ...
                    "perplexity": round(float(perplexity), 6),
                    "ttr_percent": round(float(ttr), 6),
                    "model1_ai": round(float(diagnostics.get("model1_ai", 0.0)), 6),
                    "model2_ai": round(float(diagnostics.get("model2_ai", 0.0)), 6),
                    "fakespot_ai": round(float(diagnostics.get("fakespot_ai", 0.0)), 6),
                }
                ...
                try:
                    (
                        ...
                        .insert(
                            {
                                # Reuse the existing table schema: no DB migration required.
                                "actual_label": ground_truth,
                                "predicted_score": float(production_score),
                                "burstiness_cv": float(burstiness),
                                "user_notes": "[PB1] " + json.dumps(
                                    evidence_record,
                                    ensure_ascii=False,
                                    separators=(",", ":"),
                                ),
                    ...
                    st.success(
                        f"Thank you — product evidence saved as {case_id}. "
                        "The analyzed text was not stored."
                    )
                except Exception as e:
                    st.error(f"Failed to save product feedback: {e}")
